...
export default function RegisterPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (username.trim() !== "user123") {
      setError(
        'Registration is restricted. Only "user123" can register in this demo.',
      );
      return;
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-zinc-950 px-4">
      ...
      <div className="w-full max-w-md relative">
        <Link href="/" className="block mb-10 text-center">
          <span className="text-3xl font-extrabold bg-gradient-to-r from-teal-500 to-emerald-500 bg-clip-text text-transparent tracking-tighter">
            Neuraforge.
          </span>
        </Link>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-gray-50 dark:bg-zinc-950 border border-gray-300 dark:border-zinc-800 rounded-xl text-gray-900 dark:text-zinc-200 placeholder-gray-400 dark:placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 transition-all text-sm"
                autoFocus
              />
            </div>
            {error && (
            ...
            <button
              type="submit"
              className="w-full flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-white font-bold py-3 rounded-xl transition-all active:scale-[0.98]"
            >
        ...
        <p className="text-center text-xs text-gray-400 dark:text-zinc-600 mt-6">
          Demo mode — only{" "}
          <code className="text-teal-600 dark:text-teal-500/80 bg-teal-500/5 px-1.5 py-0.5 rounded">
            user123
          </code>{" "}
          can register
        </p>
